...
?>
<ul class="nav">
	<li <?=$active['0'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=album&status=0">待审核宝贝</a></li>
	<li <?=$active['-1'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=album&status=-1">拒绝的宝贝</a></li>
	<li <?=$active['1'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=album&status=1">已审核通过</a></li>
</ul>
<div class="box-content">
	...
	<div class="table">
		...
		<div class="th">
		     <select name="aid">
		       ...
		       <option value="<?=$value['aid'];?>" <?php if(request('aid','')==$value['aid']){ ?>selected<?php } ?>>
		       		<?=$value['title'];?>
		       	</option>
		       <?php } ?>
		     </select>
	...
	<div class="table">
        ...
        <tr id="data_<?=$value['id'];?>">
        	...
        	<td class="text-center"><?=$album_list[$value['aid']]['title'];?></td>
        	...
        	<td class="text-center">
        		<?php if(!empty($value['pay_money'])){ ?><img src="static/images/tick.gif">
        	...
        	<td class="text-center">
            	...
            	[<a href="<?=$_extend_url;?>&pmod=apply&op=album&do=edit&status=<?=$status;?>&id=<?=$value['id'];?>">修改</a>]&nbsp;
            	<?php }else{ ?>
	            	<?php if($value['status']==0){ ?>
        ...
        <?php if($value['status']==-1){ ?>
        <!--//拒绝理由-->
        <tr><td colspan="15">拒绝理由:<?=$value['refuse'];?></td></tr>
        <?php } ?>
        <?php } ?>
        </tbody></table>
	</div>
