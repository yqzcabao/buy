...
?>
<ul class="nav">
	<li <?=$active['0'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=special&status=0">待审核宝贝</a></li>
	<li <?=$active['-1'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=special&status=-1">拒绝的宝贝</a></li>
	<li <?=$active['1'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=special&status=1">已审核通过</a></li>
</ul>
	...
	<div class="table">
		...
		<div class="th">
		     <select name="aid">
		       ...
		       <option value="-<?=$value['sid'];?>" <?php if(request('aid','')==-$value['sid']){ ?>selected<?php } ?>>
		       		<?=$value['title'];?>
		       	</option>
		       <?php } ?>
		     </select>
	...
	<div class="table">
        ...
        <tr id="data_<?=$value['id'];?>">
        	...
        	<td class="text-center">
            	...
            	[<a href="<?=$_extend_url;?>&pmod=apply&op=special&do=edit&status=<?=$status;?>&id=<?=$value['id'];?>">修改</a>]&nbsp;
            	<?php }else{ ?>
	            	<?php if($value['status']==0){ ?>
