...
<ul class="nav">
	<li <?=$active['all'];?>><a href="<?=$_extend_url;?>&pmod=log&op=withdraw&s=all">全部</a></li>
	<li <?=$active['audit'];?>><a href="<?=$_extend_url;?>&pmod=log&op=withdraw&s=audit">待审核</a></li>
	<li <?=$active['succeed'];?>><a href="<?=$_extend_url;?>&pmod=log&op=withdraw&s=succeed">已成功</a></li>
</ul>
	...
	<form method="POST" action="<?=$_extend_url;?>&pmod=del" onsubmit="return confirmdel();">
	    ...
	    <table class="admin-tb"><tbody>
		    <tr>
		    	...
		    	<th width="60">用户</th>
		        <th width="50">金额</th>
		    	<th width="100" class="text-center">提现时间</th>
		    	<th width="100" class="text-center">成功时间</th>
		    	<th width="50" class="text-center">提现方式</th>
		    	<th width="50" class="text-center">账号</th>
		    	<th width="100" class="text-center">支付宝交易号</th>
		    ...
		    <tr>
		        ...
		        <td class="text-center red">
		        	...
		        	<?php }elseif ($value['status']==0){ ?>待处理<?php } ?>
		        </td>
		        <td class="text-center">
		        	<?php if($value['status']==0){ ?>
		        	<a href="javascript:void(0);" id="serialno_<?=$value['serialno'];?>" onclick="withdraw_audit('<?=$value['serialno'];?>','<?=$value['money'];?>','<?=$value['method'];?>','<?=$value['account'];?>');">审核</a>
		        	<?php } ?>
		        </td>
		    </tr>
		    <?php } ?>
